#!/usr/bin/env bash

cd $PWD/kaldi

cmd="perl queue.pl"
nj=4
vad_loc="./vad"
vad_config="./vad/vad.conf"

if [ $# -lt 3 ] || [ $# -gt 3 ]; then
   echo "Usage: $0 [options] <data-loc> <split> <params-file>";
   echo "e.g.: $0 data/train_data train_data mfcc.params"
   exit 1;
fi

source $3
data_loc=$1
feats_scp=${data_loc}/feats.scp
name=$2

log_loc="$vad_loc/log"
mkdir -p ${vad_loc} || exit 1;
mkdir -p ${log_loc}|| exit 1;

python split_scp.py --splits ${nj} --file ${feats_scp} --prefix feats --dest ${log_loc} || exit 1;

${cmd} JOB=1:${nj} ${log_loc}/vad.${name}.JOB.log \
  compute-vad --config=${vad_config} scp:${log_loc}/feats.JOB.scp \
  ark,scp:${vad_loc}/vad.${name}.JOB.ark,${vad_loc}/vad.${name}.JOB.scp || exit 1;

for ((n=1; n<=nj; n++)); do
  cat ${vad_loc}/vad.${name}.${n}.scp || exit 1;
done > ${data_loc}/vad.scp